...
from windows.cD_window import cD_window_class
from windows.mQD_window import mQD_window_class
from windows.fHMD_window import fhmd_window_class
...
# ------------------------------------Второй блок формы--------------------------------------#
		...
		self.fhm = QtGui.QRadioButton("foamyHexMesh")
		...
		self.mesh_type_vbox.addWidget(self.fhm)
	...
	def on_chc_clicked(self):
		...
		collapseDict_path = prj_dir + '/' + 'collapseDict.pkl'
		meshQualityDict_path = prj_dir + '/' + 'meshQualityDict.pkl'
		
		if os.path.exists(initial_path) == True or os.path.exists(collapseDict_path) == True or os.path.exists(meshQualityDict_path) == True:
			...
			if pd_2 == 'blockMesh':
				...
				par.on_mesh_type_get(pd_2)
			elif pd_2 == 'foamyHexMesh':
				self.fhm.setChecked(True)
				par.on_mesh_type_get(pd_2)
	...
	def on_save_clicked(self):
		...
		if self.radio_1.isChecked():
			...
			if int_lng == 'Russian':
				ldw_label.setText("Форма генерации расчетной сетки: " + "<font color='peru'>" + mesh_name_txt + "_" + pd_2 + "</font>")
			elif int_lng == 'English':
				ldw_label.setText("Form of mesh generation: " + "<font color='peru'>" + mesh_name_txt + "_" + pd_2 + "</font>")
			...
			par.msf_menu = par.menu_bar.addMenu('Служебные файлы сетки foamyHexMesh')
						
			par.msf_menu.addAction(par.fhmd_open_btn)
			par.fhmd_open_btn.triggered.connect(self.on_fhmd_open)

			par.msf_menu.addAction(par.mQD_open_btn)
			par.mQD_open_btn.triggered.connect(self.on_mQD_open)

			par.msf_menu.addAction(par.cD_open_btn)
			par.cD_open_btn.triggered.connect(self.on_cD_open)
			...
			if self.nf_radio.isChecked() == True:
				if self.mesh_name.text() == '':
					...
					if self.bm.isChecked() == True:
						...
						par.addDockWidget(QtCore.Qt.BottomDockWidgetArea, par.serv_mes)
						pckls_path = full_dir + '/system/'
						pd_2_cur = 'foamyHexMesh'
						fhmd_form = fhmd_window_class(self, par, pckls_path, mesh_name_txt, pd_2_cur)
						par.ldw.setWidget(fhmd_form)
						self.close()
						...
						fqmd_form = fqmd_window_class(self, par, pckls_path, mesh_name_txt, pd_2_cur)
				...
				if pd_2 == 'blockMesh':
					par.addDockWidget(QtCore.Qt.BottomDockWidgetArea, par.serv_mes)
					pd_2_cur = 'blockMesh'
					bmd_form = bmd_window_class(self, par, prj_path_cur, mesh_name_txt, pd_2_cur)
					par.ldw.setWidget(bmd_form)

					if os.path.exists(prj_path_cur + '/blockMeshDict'):
						outf = open(prj_path_cur + '/blockMeshDict')
						data = outf.read()
						if int_lng == 'Russian':
							par.outf_lbl.setText("Файл " + "<font color='peru'>" + 'blockMeshDict' + "</font>") 
						elif int_lng == 'English':
							par.outf_lbl.setText("<font color='peru'>" + 'blockMeshDict' + "</font>" + " file")
					...
					par.addDockWidget(QtCore.Qt.BottomDockWidgetArea, par.serv_mes)
					pd_2_cur = 'snappyHexMesh'
					shmd_form = shmd_window_class(self, par, prj_path_cur, mesh_name_txt, pd_2_cur)
					par.ldw.setWidget(shmd_form)

					if os.path.exists(prj_path_cur + '/snappyHexMeshDict'):
						outf = open(prj_path_cur + '/snappyHexMeshDict')
						data = outf.read()
						if int_lng == 'Russian':
					...
					pd_2_cur = 'foamyHexMesh'
					fhmd_form = fhmd_window_class(self, par, prj_path_cur, mesh_name_txt, pd_2_cur)
					par.ldw.setWidget(fhmd_form)

					if os.path.exists(prj_path_cur + '/foamyHexMeshDict'):
						outf = open(prj_path_cur + '/foamyHexMeshDict')
						data = outf.read()
						if int_lng == 'Russian':
							par.outf_lbl.setText("Файл " + "<font color='peru'>" + 'foamyHexMeshDict' + "</font>") 
						elif int_lng == 'English':
							par.outf_lbl.setText("<font color='peru'>" + 'foamyHexMeshDict' + "</font>" + " file") 
						par.outf_edit.setText(data)
						par.cdw.setWidget(par.outf_scroll)
					...
					par.addDockWidget(QtCore.Qt.BottomDockWidgetArea, par.serv_mes)
					pd_2_cur = 'foamyQuadMesh'
					fqmd_form = fqmd_window_class(self, par, prj_path_cur, mesh_name_txt, pd_2_cur)
					par.ldw.setWidget(fqmd_form)

					if os.path.exists(prj_path_cur + '/foamyQuadMeshDict'):
						outf = open(prj_path_cur + '/foamyQuadMeshDict')
						data = outf.read()
						if int_lng == 'Russian':
							par.outf_lbl.setText("Файл " + "<font color='peru'>" + 'foamyQuadMeshDict' + "</font>") 
						elif int_lng == 'English':
							ar.outf_lbl.setText("<font color='peru'>" + 'foamyQuadMeshDict' + "</font>" + " file") 
						par.outf_edit.setText(data)
						par.cdw.setWidget(par.outf_scroll)
				...
				if int_lng == 'Russian':
					msg = 'Загружены параметры сетки ' + self.mesh_name.text() + '. Установите ее в качестве текущей, выполнив генерацию сетки'
				elif int_lng == 'English':
					msg = 'Mesh parameters are loaded ' + self.mesh_name.text() + '. Set it as current by a mesh generation'
				par.listWidget.clear()
	...
	def on_fhmd_open(self):
		...
		ldw_label = QtGui.QLabel()
		if int_lng == 'Russian':
			ldw_label.setText("Форма генерации файла: " + "<font color='peru'>" + 'foamyHexMeshDict' + "</font>")
		elif int_lng == 'English':
			ldw_label.setText("Generation form of file: " + "<font color='peru'>" + 'foamyHexMeshDict' + "</font>")
				
	def on_cD_open(self):
		pckls_path = full_dir + '/system'
		pd_2_cur = 'foamyHexMesh'
		cD_form = cD_window_class(self, par, pckls_path, mesh_name_txt, pd_2_cur)
		par.ldw.setWidget(cD_form)
		if self.cf_radio.isChecked() == True:
			
			if os.path.exists(prj_path_cur + '/collapseDict'):
				par.cdw.setWidget(par.outf_scroll)
				par.cdw.setTitleBarWidget(par.cdw_frame)
				
				outf = open(prj_path_cur + '/collapseDict') 
				data = outf.read()
				par.outf_edit.setText(data)
		...
		if int_lng == 'Russian':
			par.outf_lbl.setText("Файл: " + "<font color='peru'>" + 'collapseDict' + "</font>") 
		elif int_lng == 'English':
			par.outf_lbl.setText("<font color='peru'>" + 'collapseDict' + "</font>" + " file") 
		
		ldw_label = QtGui.QLabel()
		if int_lng == 'Russian':
			ldw_label.setText("Форма генерации файла: " + "<font color='peru'>" + 'collapseDict' + "</font>")
		elif int_lng == 'English':
			ldw_label.setText("Generation form of file: " + "<font color='peru'>" + 'collapseDict' + "</font>")
		
	def on_mQD_open(self):
		pckls_path = full_dir + '/system'
		pd_2_cur = 'foamyHexMesh'
		mQD_form = mQD_window_class(self, par, pckls_path, mesh_name_txt, pd_2_cur)
		par.ldw.setWidget(mQD_form)
		
		if self.cf_radio.isChecked() == True:
			
			if os.path.exists(prj_path_cur + '/meshQualityDict'):
				par.cdw.setWidget(par.outf_scroll)
				par.cdw.setTitleBarWidget(par.cdw_frame)
				
				outf = open(prj_path_cur + '/meshQualityDict')
				data = outf.read()
				par.outf_edit.setText(data)
		...
		if int_lng == 'Russian':
			par.outf_lbl.setText("Файл: " + "<font color='peru'>" + 'meshQualityDict' + "</font>") 
		elif int_lng == 'English':
			par.outf_lbl.setText("<font color='peru'>" + 'meshQualityDict' + "</font>" + " file") 
		
		ldw_label = QtGui.QLabel()
		if int_lng == 'Russian':
			ldw_label.setText("Форма генерации файла: " + "<font color='peru'>" + 'meshQualityDict' + "</font>")
		elif int_lng == 'English':
			ldw_label.setText("Generation form of file: " + "<font color='peru'>" + 'meshQualityDict' + "</font>")
